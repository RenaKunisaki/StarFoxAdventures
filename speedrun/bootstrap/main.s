# Contents of boot.bin, which replaces LACTIONS.bin.
# The game loads that file at startup, and the patched DOL will jump into it,
# as a sort of "second-stage bootloader". It must be position-independent code.
...
offsGOT:   .int 0xDEADBEEF # offset of GOT (-4 for lwzu)
sizeGOT:   .int 0xDEADBEEF # size of GOT in words
offsEntry: .int 0xDEADBEEF # entry point of ELF
sizeBoot:  .int 0xDEADBEEF # size of this bootstrap code
...
_start2:
    ...
    # enable debug text now in case of crash
    li      r3,  1
...
.if DEBUG
    s_hello:        .string "hello, cruel world. addr 0x%X size 0x%X SP=0x%X"
