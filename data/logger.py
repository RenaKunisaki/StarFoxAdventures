...
def setup(appName, logfile="debug.log"):
    ...
    class MyFormatter(logging.Formatter):
        ...
        def format(self, record):
            ...
            if record.threadName == 'MainThread':
                ...
                if tid in _threads: fid = _threads[tid]
                else:
                    fid = len(_threads) + 1
                    _threads[tid] = fid
                record.threadNameFmt = '\x1B[38;5;6m:%s:%d' % (record.threadName, fid)
    ...
    if logfile is not None:
        fh = logging.FileHandler(logfile, mode='wt')
