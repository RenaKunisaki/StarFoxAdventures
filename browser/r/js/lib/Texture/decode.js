...
block_data_size, colors) {
    ...
    for(let byte_index=0; byte_index<block_data_size; byte_index++) {
        const byte = read_u8(image_data, offset+byte_index);
...
block_data_size, colors) {
    ...
    for(let i=0; i<block_data_size; i++) {
        pixel_color_data.push(convert_i8_to_color(read_u8(
            image_data, offset+i)));
    }
    return pixel_color_data;
}
...
block_data_size, colors) {
    ...
    for(let i=0; i < Math.trunc(block_data_size/2); i++) {
        pixel_color_data.push(convert_ia8_to_color(read_u16(
            image_data, offset+i*2)));
    }
    return pixel_color_data;
}
...
block_data_size, colors) {
    const pixel_color_data = [];
    for(let i=0; i<Math.trunc(block_data_size/2); i++) {
        pixel_color_data.push(convert_rgb565_to_color(read_u16(
            image_data, offset+i*2)));
    }
    return pixel_color_data;
}
...
block_data_size, colors) {
    const pixel_color_data = [];
    for(let i=0; i<Math.trunc(block_data_size/2); i++) {
        pixel_color_data.push(convert_rgb5a3_to_color(read_u16(
            image_data, offset+i*2)));
    }
    return pixel_color_data;
}
...
block_data_size, colors) {
    ...
    for(let i=0; i<16; i++) {
        pixel_color_data.push([
            read_u8(image_data, offset+(i*2)),
            read_u8(image_data, offset+(i*2)+1),
            read_u8(image_data, offset+(i*2)+32),
            read_u8(image_data, offset+(i*2)+33),
        ]);
    }
    return pixel_color_data;
}
...
function decode_c8_block(image_format, image_data, offset, block_data_size, colors) {
    const pixel_color_data = [];
    for(let i=0; i<block_data_size; i++) {
        const color_index = read_u8(image_data, offset+i);
        if(color_index >= colors.length) {
            //This block bleeds past the edge of the image
            color = null;
        }
...
function decode_c14x2_block(image_format, image_data, offset, block_data_size, colors) {
    const pixel_color_data = [];
    for(let i=0; i<Math.trunc(block_data_size/2); i++) {
        const color_index = read_u16(image_data, offset+i*2) & 0x3FFF;
        if(color_index >= colors.length) {
            //This block bleeds past the edge of the image
            color = null;
        }
