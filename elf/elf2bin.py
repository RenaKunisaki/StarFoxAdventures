#!/usr/bin/env python3
"""Read one or more ELF files and produce PATCHES.BIN and PATCHES.TAB."""
import os
import sys
import struct
...
def main():
    ...
    if len(args) < 2:
        print("Usage: elf2bin.py outPath file.elf file.elf...")
        sys.exit(1)

    outPath = args.pop(0)
    outBin  = open(os.path.join(outPath, 'PATCHES.BIN'), 'wb')
    outTab  = open(os.path.join(outPath, 'PATCHES.TAB'), 'wb')
    ...
    while len(args) > 0:
        ...
        for shdr in elf.sectHdrs:
            ...
            if name == '.got':
                # 0x54 is ELF header size.
                # 0x10 is header before our code.
                # XXX replace 0x54 with the file offset of the section.
                gotOffs = (shdr['offset'] - 0x54) + 0x10
                gotSize = shdr['size'] >> 2
                break
